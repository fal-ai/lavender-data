...
1. **Main Shard**: This is the primary data source that contains the core information and serves as the reference point for joining. 
    The shardset with the most samples is chosen as the main shard. If there are multiple shardsets with the same number of samples,
    the one with the oldest creation date is chosen as the main shard.
