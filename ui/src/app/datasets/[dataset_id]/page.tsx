...
async function DatasetPreview({
  ...
  if (!preview) {
    return (
      <ErrorCard
        error={`Preview is not available for this dataset. ${previewResponse?.error?.detail ? previewResponse.error.detail : ''}`}
      />
    );
  }
...
export default async function DatasetDetailPage({
  ...
  const { preview_page = 0, tab = 'preview' } = await searchParams;
